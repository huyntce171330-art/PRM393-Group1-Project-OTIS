...
void main() async {
  ...
  // ========== 1. USER_ROLES ==========
  sql.writeln('DROP TABLE IF EXISTS user_roles;');
  sql.writeln(
    'CREATE TABLE user_roles (role_id INTEGER PRIMARY KEY AUTOINCREMENT, role_name TEXT UNIQUE NOT NULL);',
  );

  // ========== 2. USERS ==========
  sql.writeln('DROP TABLE IF EXISTS users;');
  sql.writeln('''CREATE TABLE users (
  ...
  // ========== 3. BRANDS ==========
  sql.writeln('DROP TABLE IF EXISTS brands;');
  sql.writeln(
    'CREATE TABLE brands (brand_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE NOT NULL, logo_url TEXT);',
  );

  // ========== 4. TIRE_SPECS ==========
  sql.writeln('DROP TABLE IF EXISTS tire_specs;');
  sql.writeln(
    'CREATE TABLE tire_specs (tire_spec_id INTEGER PRIMARY KEY AUTOINCREMENT, width INTEGER, aspect_ratio INTEGER, rim_diameter INTEGER);',
  );

  // ========== 5. PRODUCTS ==========
  sql.writeln('DROP TABLE IF EXISTS products;');
  sql.writeln('''CREATE TABLE products (
  ...
  // ========== 6. CART_ITEMS ==========
  sql.writeln('DROP TABLE IF EXISTS cart_items;');
  sql.writeln(
    'CREATE TABLE cart_items (user_id INTEGER, product_id INTEGER, quantity INTEGER, PRIMARY KEY (user_id, product_id));',
  );

  // ========== 7. ORDERS ==========
  sql.writeln('DROP TABLE IF EXISTS orders;');
  sql.writeln('''CREATE TABLE orders (
  ...
  // ========== 8. ORDER_ITEMS ==========
  sql.writeln('DROP TABLE IF EXISTS order_items;');
  sql.writeln(
    'CREATE TABLE order_items (order_id INTEGER, product_id INTEGER, quantity INTEGER, unit_price REAL, PRIMARY KEY (order_id, product_id));',
  );

  // ========== 9. NOTIFICATIONS ==========
  sql.writeln('DROP TABLE IF EXISTS notifications;');
  sql.writeln('''CREATE TABLE notifications (
  ...
  // ========== 10. CHAT_ROOMS ==========
  sql.writeln('DROP TABLE IF EXISTS chat_rooms;');
  sql.writeln(
    'CREATE TABLE chat_rooms (room_id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER UNIQUE, status TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP);',
  );

  // ========== 11. MESSAGES ==========
  sql.writeln('DROP TABLE IF EXISTS messages;');
  sql.writeln(
    'CREATE TABLE messages (message_id INTEGER PRIMARY KEY AUTOINCREMENT, room_id INTEGER, sender_id INTEGER, content TEXT, image_url TEXT, is_read INTEGER DEFAULT 0, created_at TEXT DEFAULT CURRENT_TIMESTAMP);',
  );

  // ========== 12. VEHICLE_MAKES ==========
  sql.writeln('DROP TABLE IF EXISTS vehicle_makes;');
  sql.writeln(
    'CREATE TABLE vehicle_makes (make_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, logo_url TEXT);',
  );
  ...
  sql.writeln('''INSERT INTO brands (name, logo_url) VALUES 
...
(50, 'LỐP SAILUN 155/65R13 73T SH406', 2000000, 100, 4, 'https://voxethaiphung.vn/userfiles/image/1.san-pham/lop-sailun-155-65r13-73t-sh406/lop-sailun155-70-13-768x768_300x300.png', 4, 4);''',
  ...
  sql.writeln('''INSERT INTO cart_items (user_id, product_id, quantity) VALUES 
(2, 1, 2), (2, 5, 1), (3, 6, 3), (3, 9, 1), (4, 3, 2);''');
...
(1, 1, 2, 1510000), (1, 5, 1, 1550000),
(2, 6, 2, 1640000), (3, 3, 1, 1530000), (3, 4, 1, 1540000);''',
  );

  sql.writeln('''INSERT INTO notifications (title, body, user_id) VALUES 
...
('Khuyen mai', 'Giam 10% cho lop Michelin', 2);''');
  ...
  sql.writeln('''INSERT INTO chat_rooms (user_id, status) VALUES 
(2, 'open'), (3, 'open'), (4, 'open');''');

  sql.writeln('''INSERT INTO messages (room_id, sender_id, content) VALUES 
...
(3, 1, 'Dang tu 1-2 ngay a');''');
  ...
  sql.writeln('''INSERT INTO vehicle_makes (name, logo_url) VALUES 
('Toyota', 'toyota.png'), ('Honda', 'honda.png'), ('Mazda', 'mazda.png'),
('Ford', 'ford.png'), ('Hyundai', 'hyundai.png');''');

  // Write SQL to temp file
  final tempSqlFile = 'temp_create_db.sql';
  File(tempSqlFile).writeAsStringSync(sql.toString());

  // Execute using sqlite3
  final result = await Process.run('sqlite3', [dbPath, '.read $tempSqlFile']);
  ...
  print('📊 Products count: ${countResult.stdout}');
